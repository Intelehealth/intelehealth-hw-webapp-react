...
export const QuestionLoader = ({
  ...
  const handleOptionClick = (option: PhysicalExamOption) => {
    ...
    if (option['input-type'] === 'camera') {
      ...
      const isExclusive =
        option['is-exclusive-option'] === 'true' ||
        option['is-exclusive-option'] === true;
        ...
        {loading ? (
          ...
          <>
            <p className="text-lg font-medium text-gray-900 leading-[156%]">
              {question}
              {showAsterisk && question && (
                <span className="text-red-500">*</span>
              )}
            </p>

            {/* Reference Images Section */}
            {physicalExamQuestion?.jobAidType === 'image' &&
              physicalExamQuestion?.jobAidFile && (
                <div className="mt-3">
                  ...
                  <div className="flex gap-3">
                    {getReferenceImages(physicalExamQuestion.jobAidFile).map(
                      (imagePath, index) => (
                        <div
                          key={index}
            ...
            {physicalExamQuestion?.jobAidType === 'video' &&
              physicalExamQuestion?.jobAidFile && (
              ...
              <>
                <div className="mt-4 option-group">
                  ...
                  {physicalExamQuestion.options
                    ...
                    .map(option => {
                      const selected =
                        isSelected(option.id) || showImageGallery;
                ...
                {showImageGallery &&
                  ...
                  physicalExamQuestion.options.some(
                    opt => opt['input-type'] === 'camera'
                  ) && (
                    <div className="mt-4">
