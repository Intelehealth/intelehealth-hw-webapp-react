...
export const profileService = {
  ...
  // GET /providerattributetype - Get provider attribute types (from provider.service.ts)
  getProviderAttributeTypes: () =>
    OpenMRSApi.get(API_ENDPOINTS.PROVIDER_ATTRIBUTE_TYPE),

  // POST /person/{uuid} - Update person (gender, age, birthdate) (from provider.service.ts)
  updatePerson: (
    personUuid: string,
    data: { gender: string; age: number; birthdate: string }
  ) => OpenMRSApi.post(`${API_ENDPOINTS.PERSON}/${personUuid}`, data),

  // POST /person/{uuid}/name - Create person name (from provider.service.ts)
  createPersonName: (
    personUuid: string,
  ...
  // POST /person/{uuid}/name/{nameUuid} - Update person name (from provider.service.ts)
  updatePersonName: (
    personUuid: string,
    nameUuid: string,
    data: { givenName: string; middleName: string; familyName: string }
  ...
  // POST /provider/{uuid}/attribute or /provider/{uuid}/attribute/{attrUuid} - Add/Update provider attribute (from provider.service.ts)
  // This combines both add and update - if attrUuid exists, it updates; otherwise, it adds
  ...
  updateProfileImage: (data: { person: string; base64EncodedImage: string }) =>
    OpenMRSApi.post(API_ENDPOINTS.PERSON_IMAGE, data, {
      ...
      headers: { Authorization: `Basic ${btoa(`nurse1:Nurse@123`)}` },
    }),
  ...
  // Execute multiple requests in parallel (equivalent to forkJoin from provider.service.ts)
